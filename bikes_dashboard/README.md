...
The app lives in `bikes_dashboard/` inside the AM01 course repo and is
self-contained: its own `pyproject.toml` and `uv.lock` pin exactly what it
needs, so the teaching environment does not pick up a web server it has no use
for. Requires [uv](https://docs.astral.sh/uv/) and Python 3.12 (uv fetches it).
...
It also runs on the course environment from the repo root — verified against
its dash 4.4.0 / plotly 6.9.0 / pandas 2.3.3 — if you would rather use the one
environment for the notebook and the app together:

```bash
uv run python bikes_dashboard/app.py
```
...
`render.yaml` sets `rootDir: bikes_dashboard`, so Render builds and runs from
this folder rather than the repo root.

1. Push to a GitHub repo you control and make it **public**.
2. On [render.com](https://render.com), choose **New → Blueprint**, connect the
   repo and apply. Render reads `render.yaml`.
3. Or, without the blueprint: **New → Web Service**, connect the repo, set
   **Root Directory** to `bikes_dashboard`, **Build** to
   `pip install uv && uv sync` and **Start** to
